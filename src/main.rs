...
const CHUNK_SIZE_AVG_DEFAULT: u64 = 64 * 1024;
const CHUNK_SIZE_MIN_DEFAULT: u64 = CHUNK_SIZE_AVG_DEFAULT / 4;
const CHUNK_SIZE_MAX_DEFAULT: u64 = CHUNK_SIZE_AVG_DEFAULT * 4;

fn main() {
    let filename = "test_files/temp/input";
    ...
    match File::open(filename) {
        Ok(mut file) => {
            ...
            loop {
                ...
                read_min(&mut file, &mut chunk_buf, &mut window_rev);
        ...
        Err(e) => {
            panic!("Could not open file for chunking {:?}", e)
        }
    }
}

fn read_min(f: &mut File, buf: &mut VecDeque<u8>, window: &mut Vec<u8>) {
